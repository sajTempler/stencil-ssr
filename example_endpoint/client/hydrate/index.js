...
var require$$0__default$2 = /*#__PURE__*/_interopDefaultLegacy(require$$0$1);
...
const deferToConnect = (socket, fn) => {
    ...
    const onConnect = () => {
        ...
        if (socket instanceof tls__default['default'].TLSSocket && hasSecureConnectListener) {
...
const nodejsMajorVersion = Number(process__default['default'].versions.node.split('.')[0]);
const timer = (request) => {
    const timings = {
        start: Date.now(),
        socket: undefined,
...
const URLParser = typeof URL === 'undefined' ? url_1__default['default'].URL : URL;
...
var eos = function(stream, opts, callback) {
	...
	var onclose = function() {
		process__default['default'].nextTick(onclosenexttick);
	};
...
var noop$1 = function () {};
var ancient = /^v?\.0/.test(process__default['default'].version);
...
var bufferStream = options => {
	...
	stream.getBufferedValue = () => {
		...
		return isBuffer ? require$$0$1.Buffer.concat(chunks, length) : chunks.join('');
	};
...
const Readable = require$$0__default$1['default'].Readable;
...
class Response extends Readable {
	constructor(statusCode, headers, body, url) {
		...
		if (!(body instanceof require$$0$1.Buffer)) {
...
var stringify = function stringify (o) {
  ...
  if(o && require$$0$1.Buffer.isBuffer(o))
  ...
  return JSON.parse(s, function (key, value) {
    if('string' === typeof value) {
      if(/^:base64:/.test(value))
        return require$$0$1.Buffer.from(value.substring(8), 'base64')
      else
...
const {Transform, PassThrough: PassThrough$1} = require$$0__default$1['default'];
...
class Agent extends EventEmitter__default['default'] {
...
const {Readable: Readable$1} = require$$0__default$1['default'];
...
class ClientRequest extends Writable {
	constructor(input, options, callback) {
		...
		if (options.auth && !('authorization' in this[kHeaders])) {
			this[kHeaders].authorization = 'Basic ' + require$$0$1.Buffer.from(options.auth).toString('base64');
		}
	...
	abort() {
		...
		if (!this.aborted) {
			process__default['default'].nextTick(() => this.emit('abort'));
		}
		...
		const onStream = stream => {
			...
			stream.once('response', waitForEnd((headers, flags, rawHeaders) => {
				...
				if (isConnectMethod) {
					...
					if (this.emit('connect', response, stream, require$$0$1.Buffer.alloc(0))) {
						this.emit('close');
					} else {
...
var resolveAlpn = (options = {}) => new Promise((resolve, reject) => {
	const socket = tls__default['default'].connect(options, () => {
...
exports.default = async (body, headers) => {
    ...
    if (dist.default.string(body)) {
        return require$$0$1.Buffer.byteLength(body);
    }
...
const getBuffer = async (stream) => {
    ...
    for await (const chunk of stream) {
        chunks.push(chunk);
        length += require$$0$1.Buffer.byteLength(chunk);
    }
    if (require$$0$1.Buffer.isBuffer(chunks[0])) {
        return require$$0$1.Buffer.concat(chunks, length);
    }
    return require$$0$1.Buffer.from(chunks.join(''));
};
...
exports.default = (message) => {
    ...
    process__default['default'].emitWarning(`Got: ${message}`, {
...
/*@__PURE__*/getDefaultExportFromCjs(normalizeArguments_1);
...
const supportsBrotli = dist.default.string(process__default['default'].versions.brotli);
...
class Request extends require$$0__default$1['default'].Duplex {
    ...
    async _onResponseBase(response) {
        ...
        if (options.followRedirect && response.headers.location && redirectCodes.has(statusCode)) {
            ...
            try {
                ...
                const redirectBuffer = require$$0$1.Buffer.from(response.headers.location, 'binary').toString();
    ...
    _writeRequest(chunk, encoding, callback) {
        ...
        this._progressCallbacks.push(() => {
            this[kUploadedSize] += require$$0$1.Buffer.byteLength(chunk, encoding);
...
/*@__PURE__*/getDefaultExportFromCjs(asPromise_1);
...
var got = /*@__PURE__*/getDefaultExportFromCjs(source$4);
...
class MyComponent {
  ...
  render() {
    var _a;
    return hAsync("div", null, (_a = this.people) === null || _a === void 0 ? void 0 : _a.map(person => hAsync("p", null, person === null || person === void 0 ? void 0 : person.name)));
  }
